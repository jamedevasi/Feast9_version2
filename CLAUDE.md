...
Not yet built — all part of the same build, not a separate later project: dashboard widgets, reports/analytics, DPDP Phase 2 (data-rights requests), login-screen branding/settings, backups, PDF generation, Excel import, doctors/procedure-types admin UI (they exist as seeded lookup data only), **plus** Google sign-in (explicitly optional/secondary per §14 itself — local password+TOTP remains the required baseline regardless), recurring appointments, structured dental charting, patient portal, automated off-site backup, prescription history (cross-case view), DPDP Phase 3 (breach log/retention sweep) — see `feast9_v2_agents.md` §14 for full detail on this last group. None of §14 is deferred or optional (Google sign-in aside, which the spec itself marks optional); it's unsequenced, not out of scope.
